...
def table3():
    ...
    for i in range(4, 9):
        print(f"\\toolsigma,\originalchip,{i},{to_percent(sba_our_search[i])},--")
        print(f"\\toolnormal,\originalchip,{i},{to_percent(sba_our_search_mean[i])},{to_percent((sba_our_search[i] - sba_our_search_mean[i]) / sba_our_search[i])}")
        print(f"\\tooleval,\originalchip,{i},{to_percent(our_res[i])},{to_percent((sba_our_search[i] - our_res[i]) / sba_our_search[i])}")
