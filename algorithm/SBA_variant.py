...
def level_inference(BER):
    ...
    for tmin in range(0, 60):
        ...
        Rlow, Rhigh = getReadRange(RelaxDistr, BER, (tmin+tmax) / 2)
...
def getReadRange(vals, BER, mean):
    # The read range [Rmin, Rmax) -- any point within [Rmin, Rmax) are within this level
    std = np.std(vals)
    distr = np.random.normal(mean, std, 1000)
    sorted = np.sort(distr)
    num_discard = int(BER * len(sorted) / 2)
    return int(sorted[num_discard]), int(sorted[-num_discard]) + 1
...
def dump_to_json(level_alloc):
    ...
    write_to_json(bpc, f"../settings/{bits_per_cell}bpc_SBAvar_{date}.json")


if __name__ == "__main__":
    init_model()
    minimal_BER(4, 1e-3, 0, 1, True)
    minimal_BER(8, 1e-3)
