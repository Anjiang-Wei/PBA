import SBA_meanvariant
import numpy

outfile = "../ember_capacity2/SBAmeanvar"
...
def simulate_error(level_alloc):
    ...
    for i in range(len(level_alloc)):
        rmin, rmax, wmin, wmax = level_alloc[i]    
        d1s = SBA_meanvariant.distributions[(wmin, wmax)]
        for point in d1s:
...
def get_dala():
    ...
    for i in range(4, 9):
        if i <= 5:
            res[i] = SBA_meanvariant.minimal_BER(i, 1e-3, 0, 1, True)
        else:
            res[i] = SBA_meanvariant.minimal_BER(i, 1e-3)
    print(res)
    return res
