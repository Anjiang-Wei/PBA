...
delta_our_ber = {
    # 2bpc absolute delta (ours_ber - norm_ber), relative (ours_ber-norm_ber)/norm_ber; 3bpc ...
    "originalchip": [],
    "emberchip": [],
    "emberchiptwo": [],
}
...
delta_our_ecc = {
    # 2bpc absolute delta (ours_ecc - norm_ecc), relative (ours_ecc-norm_ecc)/norm_ecc; 3bpc ...
    "originalchip": [],
    "emberchip": [],
    "emberchiptwo": [],
}
...
delta_sba_ber = {
   # 2bpc absolute delta (sba_ber - norm_ber), relative (sba_ber-norm_ber)/norm_ber; 3bpc ...
    "originalchip": [],
    "emberchip": [],
    "emberchiptwo": [],
}
...
delta_sba_ecc = {
    # 2bpc absolute delta (sba_ecc - norm_ecc), relative (sba_ecc-norm_ecc)/norm_ecc; 3bpc ...
    "originalchip": [],
    "emberchip": [],
    "emberchiptwo": [],
}
...
def get_ber(path, level_num):
    ...
    with open(path + "log_trans") as fin:
        ...
        for line in lines:
            ...
            if "norm" + str(level_num) in line:
                mean8 = float(line.split(" : ")[-1].split(",")[0])
    return ours8, SBA8, mean8
    ...
    with open(path + "log_ecc") as fin:
        ...
        for line in lines:
            ...
            if start:
                ...
                if "norm" + str(level_num) in line:
                    norm8 = float(line.split(" : ")[-1].split(",")[1])
    return ours8, SBA8, norm8
...
def filldata(dataset_str, path):
    chip_ber_ours_4, chip_ber_SBA_4, chip_ber_norm_4 = get_ber(path, 4)
    chip_ber_ours_8, chip_ber_SBA_8, chip_ber_norm_8 = get_ber(path, 8)
    chip_ecc_ours_4, chip_ecc_SBA_4, chip_ecc_norm_4 = get_ecc(path, 4)
    chip_ecc_ours_8, chip_ecc_SBA_8, chip_ecc_norm_8 = get_ecc(path, 8)
    if chip_ber_ours_4 == 0:
        chip_ecc_ours_4 = 1
    if chip_ber_SBA_4 == 0:
        chip_ecc_SBA_4 = 1
    if chip_ber_norm_4 == 0:
        chip_ecc_norm_4 = 1
    chip_ecc_ours_4 -= 1
    chip_ecc_SBA_4 -= 1
    chip_ecc_norm_4 -= 1
